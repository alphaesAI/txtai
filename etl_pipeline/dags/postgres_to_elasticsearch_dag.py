...
def transform_data(extraction_result: Dict[str, Any], **context) -> Dict[str, Any]:
    ...
    """
    try:
        # Handle case where extraction_result might be a string or None
        if not extraction_result:
            raise ValueError("No extraction result received")
        ...
        if isinstance(extraction_result, str):
            try:
                ...
                raise ValueError(f"Failed to parse extraction result: {extraction_result}")
        
        if not isinstance(extraction_result, dict):
            raise ValueError(f"Expected dict for extraction_result, got {type(extraction_result).__name__}")
            
        table_name = extraction_result.get('table_name')
        if not table_name:
            raise ValueError("No table_name found in extraction result")
        ...
        logger.error(f"Extraction result type: {type(extraction_result).__name__}")
        logger.error(f"Extraction result content: {str(extraction_result)[:500]}")
        raise
...
with DAG(
    ...
    for table_config in CONFIG['extraction']['tables']:
        ...
        transform_task = PythonOperator(
            ...
            op_kwargs={'extraction_result': "{{ ti.xcom_pull(task_ids='extract_" + table_name + "') }}"},
            provide_context=True
        )
